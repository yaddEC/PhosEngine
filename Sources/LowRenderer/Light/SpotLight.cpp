...
void LowRenderer::SpotLight::Render(const Resource::ShaderProgram& shaderProg, int number)
{
	if (isActive)
	{
		...
		shaderProg.SetUniformFloat("spotLights[" + std::to_string(number) + "].outerCutOff", cosf(angle * 1.4f * Maths::DEG2RAD));
...
void LowRenderer::SpotLight::GUIUpdate()
{
	if (Wrapper::GUI::CollapsingHeader("Point Light"))
	{
		...
		Wrapper::GUI::EditFloat("##Intensity", intensity, 0.01f, 0.f, 3.0f);
		Wrapper::GUI::DisplayText("Angle: "); Wrapper::GUI::SameLine();
		Wrapper::GUI::EditFloat("##Angle", angle, 0.1f, 0.f, 360.0f);
