...
void main ()
{
    ...
    {
        Lo += CalcDirLight(dirLights[i], N, V, F0, metallic, roughness, albedo);
    }
    for(int i = 0; i < lenghtPointLight; i++)
    {
        Lo += CalcPointLight(pointLights[i], N, V, F0, metallic, roughness, albedo);
    }
    for(int i = 0; i < lenghtSpotLight; i++)
    {
        Lo += CalcSpotLight(spotLights[i], N, V, F0, metallic, roughness, albedo);
    }
    // for(int i = 0; i < 4; ++i) 
    // {
    //     // calculate per-light radiance
